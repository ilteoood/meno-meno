...
- [ADR 0011 — `OffertaFisso` schema confermato adeguato dal PoC Fastweb + ADR 0006 §Supported transports esteso a `commodity: 'fisso'`](docs/adr/0011-offerta-fisso-schema-adeguato.md): primo scraper Fastweb Casa (FTTH) end-to-end con live-correctness verde, schema ADR 0005 regge per fisso (a differenza del mobile che ha dovuto droppare `velocita_mbps` in ADR 0010). Pattern: landing + per-card detail page per estrarre Mbps reali. ci-live-gate aggiornato per iterare commodities per operatore (multi-commodity support). Scopo v4 map (#147).