...
import { IliadMobileScraper, IliadFissoScraper } from '../src/scrapers/iliad.ts';

const fixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'iliad', 'mobile.html');
const fissoFixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'iliad', 'fisso.html');
...
test('IliadMobileScraper parses fixture HTML into one or more offerte mobile', async () => {
...
test('IliadFissoScraper parses fixture HTML into one or more offerte fisso', async () => {
  const scraper = new IliadFissoScraper({ kind: 'fixture', path: fissoFixturePath });
  const result = await scraper.scrape();
...
test('IliadFissoScraper returns ok=false when fixture path is missing', async () => {
  const scraper = new IliadFissoScraper({ kind: 'fixture', path: '/nope/missing.html' });
  const result = await scraper.scrape();
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.error, /ENOENT|no such file/);
