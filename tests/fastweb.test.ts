...
import { FastwebFissoScraper, FastwebMobileScraper } from '../src/scrapers/fastweb.ts';

const fixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'fastweb', 'mobile.html');
const fissoFixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'fastweb', 'fisso.html');
...
test('FastwebMobileScraper parses fixture HTML into one or more offerte mobile', async () => {
  ...
  for (const o of result.offerte) {
    ...
    assert.ok(typeof o.operatore_id === 'string' && o.operatore_id.length > 0);
...
test('FastwebFissoScraper parses fixture HTML into one or more offerte fisso', async () => {
  const scraper = new FastwebFissoScraper({ kind: 'fixture', path: fissoFixturePath });
  const result = await scraper.scrape();
...
test('FastwebFissoScraper returns ok=false when fixture path is missing', async () => {
  const scraper = new FastwebFissoScraper({ kind: 'fixture', path: '/nope/missing.html' });
  const result = await scraper.scrape();
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.error, /ENOENT|no such file/);
