...
import { SkywifiMobileScraper, SkywifiFissoScraper } from '../src/scrapers/skywifi.ts';

const fixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'skywifi', 'mobile.html');
const fissoFixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'skywifi', 'fisso.html');
...
test('SkywifiMobileScraper parses fixture HTML into one or more offerte mobile', async () => {
...
test('SkywifiFissoScraper parses fixture HTML into one or more offerte fisso', async () => {
  const scraper = new SkywifiFissoScraper({ kind: 'fixture', path: fissoFixturePath });
  const result = await scraper.scrape();
...
test('SkywifiFissoScraper returns ok=false when fixture path is missing', async () => {
  const scraper = new SkywifiFissoScraper({ kind: 'fixture', path: '/nope/missing.html' });
  const result = await scraper.scrape();
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.error, /ENOENT|no such file/);
