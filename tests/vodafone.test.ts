...
import { VodafoneMobileScraper, VodafoneFissoScraper } from '../src/scrapers/vodafone.ts';

const fixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'vodafone', 'mobile.html');
const fissoFixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'vodafone', 'fisso.html');
...
test('VodafoneMobileScraper parses fixture HTML into one or more offerte mobile', async () => {
...
test('VodafoneMobileScraper returns ok=false when fixture path is missing', async () => {
  ...
  if (result.ok) return;
  assert.match(result.error, /ENOENT|no such file|browser unavailable/);
});

test('VodafoneFissoScraper parses fixture HTML into one or more offerte fisso', async () => {
  const scraper = new VodafoneFissoScraper({ kind: 'fixture', path: fissoFixturePath });
  const result = await scraper.scrape();
...
test('VodafoneFissoScraper returns ok=false when fixture path is missing', async () => {
  const scraper = new VodafoneFissoScraper({ kind: 'fixture', path: '/nope/missing.html' });
  const result = await scraper.scrape();
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.error, /ENOENT|no such file|browser unavailable/);
