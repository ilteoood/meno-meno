...
import { TiscaliMobileScraper, TiscaliFissoScraper } from '../src/scrapers/tiscali.ts';

const fixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'tiscali', 'mobile.html');
const fissoFixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'tiscali', 'fisso.html');
...
test('TiscaliFissoScraper parses fixture HTML into one or more offerte fisso', async () => {
  const scraper = new TiscaliFissoScraper({ kind: 'fixture', path: fissoFixturePath });
  const result = await scraper.scrape();
...
test('TiscaliFissoScraper returns ok=false when fixture path is missing', async () => {
  const scraper = new TiscaliFissoScraper({ kind: 'fixture', path: '/nope/missing.html' });
  const result = await scraper.scrape();
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.error, /ENOENT|no such file/);
