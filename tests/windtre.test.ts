...
import { WindtreMobileScraper, WindtreFissoScraper } from '../src/scrapers/windtre.ts';

const fixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'windtre', 'mobile.html');
const fissoFixturePath = resolve(import.meta.dirname, '..', 'fixtures', 'windtre', 'fisso.html');
...
test('WindtreMobileScraper parses fixture HTML into one or more offerte mobile', async () => {
...
test('WindtreFissoScraper parses fixture HTML into one or more offerte fisso', async () => {
  const scraper = new WindtreFissoScraper({ kind: 'fixture', path: fissoFixturePath });
  const result = await scraper.scrape();
...
test('WindtreFissoScraper returns ok=false when fixture path is missing', async () => {
  const scraper = new WindtreFissoScraper({ kind: 'fixture', path: '/nope/missing.html' });
  const result = await scraper.scrape();
  assert.equal(result.ok, false);
  if (result.ok) return;
  assert.match(result.error, /ENOENT|no such file/);
